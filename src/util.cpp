...
#else
    const char* pszModule = "MultiWalletCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MultiWalletCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MultiWalletCoin
    // Mac: ~/Library/Application Support/MultiWalletCoin
    // Unix: ~/.MultiWalletCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MultiWalletCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "MultiWalletCoin";
#else
    // Unix
    return pathRet / ".MultiWalletCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MultiWalletCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MultiWalletCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong MultiWalletCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("MultiWalletCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
