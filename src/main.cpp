...
unsigned int nStakeMinAge = 5 * 60 * 60; // 5 hour
...
const string strMessageMagic = "MultiWalletCoin Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MultiWalletCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Militants continued to fire rockets from Gaza into Israel...";
        CTransaction txNew;
        txNew.nTime = 1406751167;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 8603459864 << CBigNum(42) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xb233013e8e62a1be482ad905c2f40771ec37827ef56e1fccca5663484f5f3a12"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe5, 0xf9, 0xb8, 0xaa };
